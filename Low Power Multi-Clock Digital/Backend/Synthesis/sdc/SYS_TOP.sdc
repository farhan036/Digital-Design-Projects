...
# Created by write_sdc on Wed Oct 1 21:01:25 2025
...
create_generated_clock [get_pins clock_divider_RX/o_div_clk]  -name RX_CLK  -source [get_ports UART_CLK]  -master_clock UART_CLK  -divide_by 1  -add
...
create_generated_clock [get_pins clock_divider_TX/o_div_clk]  -name TX_CLK  -source [get_ports UART_CLK]  -master_clock UART_CLK  -divide_by 32  -add
...
group_path -name INOUT  -from [list [get_ports REF_CLK] [get_ports UART_CLK] [get_ports RST_N] [get_ports UART_RX_IN]]  -to [list [get_ports UART_TX_O] [get_ports parity_error] [get_ports framing_error]]
